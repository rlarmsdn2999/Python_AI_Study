...
# DQN의 핵심은 Replay Memory를 사용하는 것이다.
# Replay Memory는 agent가 경험한 것을 저장해두었다가, batch size만큼 랜덤하게 뽑아서 학습하는 것이다.
# 이렇게 하면, 이전에 학습한 것을 다시 사용할 수 있고, 랜덤하게 뽑아서 학습하므로, 더 안정적으로 학습할 수 있다.
# 또한, 이렇게 하면, 이전에 학습한 것을 다시 사용하므로, 학습한 것을 잊어버리는 현상을 방지할 수 있다.
...
# Input Extraction
# Environment로부터 image를 추출하여 확인. simulation을 위한 화면의 구성을 처리.
...
############# Training #############
# Hyperparameters 설정 and utilities 함수 정의
# Epsilon greedy policy를 따라 action을 선택.
# epsilon 값은 EPS_START 에서 시작 EPS_END 로 끝남.
# EPS_DECAY 는 (exponentially) 감쇠하는 속도를 조절하는 변수
BATCH_SIZE = 128
GAMMA = 0.999
EPS_START = 0.9
EPS_END = 0.05
...
############## Training Loop ##############
# Optimization을 one step 수행하는 부분
# next state가 없는 경우를 분리하여 생각해야 함
