...
while True:
    ...
    if ret == True:
        ...
        if keyinput == ord('q'): 
            ...
            print('1키가 눌렸습니다. 이 영상을 [1번폴더]에 저장합니다({}).', data1_Num)

        elif keyinput == ord('2'): 
            data2_Num += 1
            save = cv2.imwrite(train_dir2 + str(data2_Num) + ".jpg", frame, params= None)
            print('2키가 눌렸습니다. 이 영상을 [1번폴더]에 저장합니다({}).', data2_Num)

        elif keyinput == ord('3'): 
            data3_Num += 1
            save = cv2.imwrite(train_dir3 + str(data3_Num) + ".jpg", frame, params= None)
            print('3키가 눌렸습니다. 이 영상을 [3번폴더]에 저장합니다({}).', data3_Num)

        elif keyinput == ord('4'): 
            data4_Num += 1
            save = cv2.imwrite(train_dir4 + str(data4_Num) + ".jpg", frame, params= None)
            print('4키가 눌렸습니다. 이 영상을 [4번폴더]에 저장합니다({}).', data4_Num)
